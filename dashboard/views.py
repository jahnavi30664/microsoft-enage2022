...
def newface(request):
	if request.method=="POST":
		...
		connect_str="DefaultEndpointsProtocol=https;AccountName=jahnavi135;AccountKey=1p5PGWj9t7Tw5mTnMh0oqPdXaPJC4WK/Bh8MCu2qUVQH/sOFh8jLWrC4E+MisM8Re4UE/9zWkB83+AStJVq83g==;EndpointSuffix=core.windows.net"
...
def dashboard(request):
	if 'email' in request.session and 'faceconfirm' in request.session:
		if request.session['faceconfirm']:
			...
			host = url.hostname
			port = url.port"""
			mydb = psycopg2.connect(host="engagepro.postgres.database.azure.com",
									user="postgres",
									password="Jahnavi_30664",
									database="mydb")
